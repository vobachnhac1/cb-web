...
export default function WheelApprove(props) {
					...
					return (
						<Space size="middle">
							...
							<Button
								...
								onClick={() => actionRow(record, "REJECT")}
							>
								Trả lại
							</Button>
						</Space>
	...
	const actionRow = async (record, key) => {
		...
		if (key == STATE_WHEEL.START) {
			...
			const _content2 = `Thời gian bắt đầu: ${moment(
				record?.from_date_act
			).format("YYYY-MM-DD")} - Thời gian kết thúc: ${moment(
				record?.to_date_act
			).format("YYYY-MM-DD")}`;
			setContentModel({
				_tittle,
			...
			const _tittle =
				"Bạn có muốn vòng quay cài đặt lại cơ cấu trúng thưởng";
			const _content1 = `Tên vòng quay: ${record?.wheel_name}`;
			const _content2 = `Thời gian bắt đầu: ${moment(
				record?.from_date_act
			).format("YYYY-MM-DD")} - Thời gian kết thúc: ${moment(
	...
	const onStop = async (record) => {
		...
		if (!result) {
			Message.Error(
				"THÔNG BÁO",
				`VÒNG QUAY ${record.wheel_name} ĐÃ ĐƯỢC DỪNG KHẨN CẤP`
			);
			setLoading(false);
			initPage();
			return;
	...
	const onStart = async (record) => {
		...
		if (!result) {
			Message.Error(
				"THÔNG BÁO",
				`VÒNG QUAY ${record.wheel_name} ĐÃ ĐƯỢC KÍCH HOẠT`
			);
			setLoading(false);
			initPage();
			return;
	...
	const onRule = async (record) => {
		...
		if (!result) {
			Message.Error(
				"THÔNG BÁO",
				`VÒNG QUAY ${record.wheel_name} ĐÃ TRẢ VỀ BƯỚC TẠO GIẢI`
			);
			setLoading(false);
			initPage();
			return;
