...
const ModalWheelDetail = (props) => {
	...
	const [wheelDetailId, setWheelDetailId] = useState(
		record ? record.wheel_detail_id : ""
	);
	...
	const [wheelColor, setWheelColor] = useState(
		record ? record.wheel_color : "#659bc9"
	);
	const [no, setNo] = useState(record ? record.no : "");
	const [remainValue, setRemainValue] = useState(
		record ? record.remain_value : ""
	);
	const [remainNumber, setRemainNumber] = useState(
		record ? record.remain_number : ""
	);
	...
	const [wheelDetailTotalValue_update, setWheelDetailTotalValue_update] =
		useState(0);
	...
	const noWheelDetail_length = useSelector(
		gettersWheelDetail.getStateWheelDetialNo
	);
	const listWheelDetail =
		useSelector(gettersWheelDetail.getStateLoadPageWheelDetail) || [];
	const WheelNumbersegment = useSelector(
		gettersWheelDetail.getStateWheelNumbersegment
	);

	let wheelCurtValue = useSelector(gettersWheelDetail.getStateWheelCurtValue);
	let wheelTotalValue = useSelector(
		gettersWheelDetail.getStateWheelTotalValue
	);
	let wheelDetialTotalValue = useSelector(
		gettersWheelDetail.getStateWheelDetialTotalValue
	);
	...
	const onCallback = async () => {
		...
		if (!remainNumber.toString() || parseInt(remainNumber) < 0) {
			msg_error.push(
				"-Số lần trúng thưởng chưa hợp lệ hoặc chưa có nội dung"
			);
		}
		if (!no || no <= 0) {
		...
		if (!isAdd && no > WheelNumbersegment && !record.is_delete) {
			msg_error.push(
				"-Số thứ tự phải nhỏ hơn hoặc bằng " + " " + WheelNumbersegment
			);
		}
		...
		for (let i = 0; i < listSegment.length; i++) {
			if (segmentId == listSegment[i].segment_id) {
				...
				if (!listSegment[i].remain_value) {
					...
					param.remain_value =
						listSegment[i].segment_value * param.remain_number;
				}
				break;
			}
		}
		...
		if (isAdd && param.remain_value > wheelCurtValue) {
			msg_error.push(
				`-Số tiền chi tiết vòng hiện tại là: ${param.remain_value
					.toString()
					.replace(
						/\B(?=(\d{3})+(?!\d))/g,
						","
					)} VND đã vượt quá số tiền còn lại của tổng vòng quay : ${wheelCurtValue
					.toString()
					.replace(
						/\B(?=(\d{3})+(?!\d))/g,
						","
					)} VND, Vui lòng chọn lại giải thưởng hoặc số lần trúng thưởng còn lại ! `
			);
		} // trường hợp update
		else if (!isAdd) {
		...
		if (isAdd) {
			...
			const result = await dispatch(
				actionWheelDetail.insertWheelDetail(param)
			);
			let data = result;
			if (result) {
		...
		//edit
		const result = await dispatch(
			actionWheelDetail.updateWheelDetail(param)
		);
		let data = result;
		if (result) {
			callback({
	...
	const calculator = (value, segment_id) => {
		...
		if (!value || (value == 0 && isAdd)) {
			...
			for (let i = 0; i < listSegment.length; i++) {
				if (
					parseInt(segment_id_value) ===
					parseInt(listSegment[i].segment_id)
				) {
					let totalWheelDetailcur = 0;
					let totalWheelDetail =
						parseInt(value) *
						parseInt(listSegment[i].segment_value);
					...
					if (totalWheelDetail == 0) {
						...
						totalWheelDetailcur =
							parseInt(wheelTotalValue) -
							(parseInt(wheelDetialTotalValue) -
								parseInt(remainValue) +
								parseInt(totalWheelDetail));
					}
					// thay đổi giá trị state curtvalue && total chi tiet
					remainNumber_value =
						listSegment[i].segment_value == 0 ? 0 : value;
		...
		>
			<Card
				headStyle={{
					...
					fontWeight: "bold",
					textAlign: "start",
					backgroundColor: "rgb(3, 77, 162)",
				}}
				title={
					isViewsWheel
						? "Xem vòng quay"
						: isAdd
						? "Thêm chi tiết vòng quay"
						: "Cập nhật chi tiết vòng quay"
				}
				bordered={true}
							...
							<Row style={{ marginTop: 10 }}>
								...
								<Col {...layoutContent}>
									<Input
										...
										value={wheelDetailId}
										onChange={(text) =>
											setWheelDetailId(text.target.value)
										}
									/>
								</Col>
							</Row>
						...
						<Row style={{ marginTop: 10 }}>
							...
							<Col {...layoutContent}>
								<Input
									...
									style={{ width: "100%" }}
									value={remainNumber}
									disabled={segmentValue == 0 ? true : false}
									onChange={onChangeRemainNumber}
								/>
							</Col>
						</Row>
						<Row style={{ marginTop: 10 }}>
							...
							<Col {...layoutContent}>
								...
								{wheelCurtValue_update < 0 ? (
									<span style={{ color: "red" }}>
										Số tiền giải thưởng hiện tại đang lớn
										hơn số tiền còn lại của vòng quay !
									</span>
								) : (
									""
								)}
						...
						<Row style={{ marginTop: 10 }}>
							...
							<Col {...layoutContent}>
								<Input
									...
									value={wheelColor}
									onChange={(text) =>
										setWheelColor(text.target.value)
									}
								/>
							</Col>
						</Row>
